...
class SimplePackageListingParser(html.parser.HTMLParser):
    ...
    def handle_starttag(self, tag: Any, attrs: Any):
        ...
        version: packaging.version.Version = packaging.version.parse(parsed_filename.version)
        if self.version == version:
            _logger.debug("version found: %r", self.version)
        else:
            _logger.debug("version mismatch: %r vs %r", self.version, version)
            return
